# -----------------------------------------------
# Implementation of the immersed boundary model
# -----------------------------------------------
#
#  Reference:
# ------------ 
#  B. A. Camley, M. G. Lerner, R. W. Pastor., F. L. Brown: 
...
def _summand(length,height,lsd,br2,ix,iy):
    pre = 2*np.pi/length
    i2  = ix**2 + iy**2
    k2  = i2*pre**2
    k   = pre*np.sqrt(i2)  
...
def d_pbc(T,eta_f,eta_m,rad,beta,length,height,imax):
    ...
    for i in xrange(-imax,imax+1):
        for j in xrange(-imax,imax+1):
            if i*i + j*j > 0:
                dpbc += _summand(length,height,lsd,br2,i,j)
    dpbc /= eta_m*length**2
    dpbc *= m2*kB*T/2.0
    return dpbc
...
def deltaD(T,eta_f,eta_m,rad,beta,length,height,imax):
    """
    Difference of finite-size and infinite-size diffusion coefficient [nm^2/s]
    """
...
def _summand_mono(length,height,eta_f,eta_m,b,br2,ix,iy):
    lsd   = eta_m/(2*eta_f)*1.0e9
    pre = 2*np.pi/length
    i2  = ix**2 + iy**2
    k2  = i2*pre**2
    ...
    bb  = 1.0e-18*b/eta_m + 0.5*k*_csch(2*height*k)/lsd
    s   = np.exp(-k2*br2/2)
    s  *= aa/(aa**2-bb**2)
    return s
...
def d_pbc_mono(T,eta_f,eta_m,b,rad,beta,length,height,imax):
    ...
    for i in range(-imax,imax+1):
        for j in range(-imax,imax+1):
            if i*i + j*j > 0:
                dpbc += _summand_mono(length,height,eta_f,eta_m,b,br2,i,j)
    dpbc /= length**2*eta_m
    dpbc *= m2*kB*T/2.0
    return dpbc


def _integrand_mono(k,eta_f,eta_m,b,br2):
    k2  = k**2
    lsd = eta_m/(2*eta_f)*1.0e9
...
def d_inf_mono(T,eta_f,eta_m,b,rad,beta):
    ...
    eta_mono = 0.5*eta_m
    br2      = (beta*rad)**2
    I = integrate.quad(_integrand_mono, 0.0, np.inf, args=(eta_f,eta_m,b,br2))
    dinf = I[0]/(2*np.pi)
